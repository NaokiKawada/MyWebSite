...
public class BuyDetailDAO {
	...
	public static void insertBuyDetail(BuyDetailDataBeans bddb) throws SQLException {
		...
		try {
			...
			st = con.prepareStatement(
					"INSERT INTO buy_detail(buy_id,item_id,size) VALUES(?,?,?)");
			st.setInt(1, bddb.getBuyId());
			st.setInt(2, bddb.getItemId());
			st.setString(3, bddb.getSize());
			st.executeUpdate();
	...
	public ArrayList<BuyDetailDataBeans> getBuyDataBeansListByBuyId(int buyId) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				bddb.setSize(rs.getString("size"));
	...
	public static ArrayList<ItemDataBeans> getItemDataBeansListByBuyId(String buyId) throws SQLException {
		Connection con = null;
		PreparedStatement st = null;
		try {
			con = DBManager.getConnection();
